...
def front_x(words):
    #sorted(words, key=lambda s: (ord(s[0]) == ord('x')))
    words.sort()
    words.sort(key=lambda s: True if s[0] == 'x' else s[0])
    return words
...
def linear_merge(sorted1, sorted2):
    new_list = []
    while len(sorted1) and len(sorted2):
        if sorted1[0] < sorted2[0]:
            new_list.append(sorted1.pop(0))
        else:
            new_list.append(sorted2.pop(0))
    if len(sorted1) > 0:
        new_list += sorted1
    elif len(sorted2) > 0:
        new_list += sorted2
